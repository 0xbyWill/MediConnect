...
import {
  ...
  Cake, Filter, Calendar, ChevronDown, Phone, MapPin,
...
export default function Pacientes({ pacientes, onAdd, onUpdate, onDelete, highlightId, initialOpen, readOnly = false }: PacientesProps) {
  ...
  const filtered = pacientes.filter(p => {
    ...
    return matchSearch && matchConvenio && matchAniv && matchEstado && matchCidade;
  });
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: '14px 18px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid var(--gray-100)' }}>
          ...
          {showFiltroAvancado && (
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--gray-100)', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { setFiltroCidade(''); setFiltroEstado(''); setFilterAnivMes(''); setFilterConvenio(''); setSearch(''); }}
              ...
              {activeTab === 'dados' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', flexWrap: 'wrap' }}>
                    <Toggle label="Paciente menor de idade?" value={showResponsavel} onChange={setShowResponsavel} disabled={isView} />
                  </div>
