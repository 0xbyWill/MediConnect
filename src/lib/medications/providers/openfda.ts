...
type OpenFdaLabel = Record<string, unknown> & { openfda?: Record<string, unknown> };
type OpenFdaResponse = { results?: OpenFdaLabel[] };

function openFdaKey() {
  return String(import.meta.env.VITE_OPENFDA_API_KEY ?? '').trim();
}
...
function mapLabelToMedication(label: OpenFdaLabel, externalId: string): Medication {
  ...
  return {
    ...
    dosages: dosageText ? [{
      ...
      frequency: 'Consultar label FDA',
      maxDose: 'Consultar label FDA',
...
export async function searchOpenFda(query: string, limit: number) {
  const trimmed = query.trim();
  if (!trimmed) return [];
  const data = await fetchJson<OpenFdaResponse>(buildSearchUrl(trimmed, limit));
  return (data.results ?? []).map(mapLabel).slice(0, limit);
}

export async function getOpenFdaById(externalId: string) {
  const key = openFdaKey();
  const keySuffix = key ? `&api_key=${encodeURIComponent(key)}` : '';
